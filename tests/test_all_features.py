...
@test("任务状态枚举: completed 会归一为 done 且 tasks/scores 不漂移")
def test_task_status_completed_normalized():
    completed = sql("SELECT COUNT(*) FROM tasks WHERE status='completed'")
    assert completed == "0", f"tasks.status='completed' 应归一为 done，当前 {completed} 条"
    mismatches = sql("""
        SELECT COUNT(*)
        FROM tasks t JOIN scores s ON t.id=s.quest_id
        WHERE t.status IN ('done','failed','backlog') AND COALESCE(t.status,'')!=COALESCE(s.status,'')
    """)
    assert mismatches == "0", f"完成态 tasks/scores 状态不一致: {mismatches} 条"
...
@test("前端: completed 状态不会从探索/交付记录丢单")
def test_frontend_completed_status_visible():
    html = read_html()
    start = html.find("function isDoneStatus")
    end = html.find("function isAutoGenerated", start)
    block = html[start:end]
    assert "completed" in block, "isDoneStatus 未兼容 completed，历史完成任务会在 UI 丢单"


@test("前端: 奏折审批 id 不直接拼进内联 onclick")
def test_frontend_approve_suggestion_id_escaped():
    html = read_html()
    assert "approveSugg('${s.id}'" not in html, "奏折 id 直接拼进 onclick，存在属性注入风险"
    assert "JSON.stringify(String(s.id))" in html, "奏折审批按钮未用 JSON.stringify 转义 id"
...
if __name__ == "__main__":
    ...
    run_section("2. 数据完整性", [
        ...
        test_task_status_completed_normalized,
    ...
    run_section("3. 前端渲染假设", [
        ...
        test_frontend_completed_status_visible,
        test_frontend_approve_suggestion_id_escaped,
    ])
